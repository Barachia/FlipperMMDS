 ...
 */
 public class POSManager extends DefaultManager {
    ...
    private String userNounPath = "$userstates.utterance.nouns";
    ...
    public void process() {
        ...
        Record nounUtterance = getIS().getRecord(userNounPath);
        if(utterance == null){
            utterance = new DefaultRecord();
            getIS().set(userUtterancePath, utterance);
        }
        if(nounUtterance == null){
            nounUtterance = new DefaultRecord();
            getIS().set(userNounPath, nounUtterance);
        }
        if(utterance.getString("nouns") == null){
            utterance.set("nouns", getName());
        }
        if(nounUtterance.getString("timestamp") == null){
            nounUtterance.set("timestamp", "t:"+System.currentTimeMillis());
        }
        if(nounUtterance.getString("frequency") == null){
            nounUtterance.set("frequency", "0");
        }
